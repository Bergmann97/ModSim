...
sim.model.p.backlogCostsPerOrder = 0.1;
...
sim.model.networkNodes = {
  ...
  "orderExit": {typeName:"ExitNode", name:"orderExit",
    onDeparture: function (processingObject) {
      const backlogQuantity = sim.stat.networkNodes[this.name].nmrOfDepartedObjects -
          sim.stat.networkNodes[this.name].nmrOfArrivedObjects;
      console.log()
      sim.stat.revenue += sim.model.p.revenuePerOrder;
      sim.stat.manufacturingCosts += sim.model.p.manufacturingCostsPerOrder;
      sim.stat.backlogCosts += sim.model.p.backlogCostsPerOrder * (sim.time - processingObject.arrivalTime);
      return [];
    }}
};
